...
def arctan1(c, epsilon):
    ...
    if c == Decimal('1'):
        ...
        add_log(f"【arctan1】arctan(1) = π/4，π ≈ {pi_val}")
        return Main(('/', pi_val, 4), epsilon)

    # 如果 c 不在区间 [-1, 1]，使用变换公式
    if abs(c) > 1:
        # 使用公式：arctan(c) = 2 * arctan(c / (1 + sqrt(1 + c^2)))
        add_log(f"【arctan1】|{c}| > 1，使用恒等式 arctan(c) = 2 * arctan(c / (1 + sqrt(1 + c²)))")
    ...
    # 返回计算结果，通过 Main 函数处理最终结果
    add_log(f"arctan({c}) ≈ {result}（共展开 {n} 项）")
